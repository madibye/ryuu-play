...
import { StoreLike, State, GameMessage, ChooseAttackPrompt, Card, Resistance, GameLog } from '../../game';
import { Effect } from '../../game/store/effects/effect';
import { AttackEffect } from '../../game/store/effects/game-effects';
import { DealDamageEffect } from '../../game/store/effects/attack-effects';

export class SlowkingSCR extends PokemonCard {
  ...
  public reduceEffect(store: StoreLike, state: State, effect: Effect): State {
    if (effect instanceof AttackEffect && effect.attack === this.attacks[0]) {
      ...
      const topdeck: Card = player.deck.top()[0];  // This is the card we're looking at.
      ...
      if (!(topdeck instanceof PokemonCard) || (
        ...
        topdeck.tags.includes(CardTag.RADIANT_POKEMON)
      )) {
        return state;
      }
